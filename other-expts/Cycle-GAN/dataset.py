from utils import common, audio_utils
import argparse
import os
import numpy as np
import glob
...
config = common.parse_yaml(args.config_path)
common.start_log(config)
...
def main():
    ...
    if not os.path.exists(clean_dir) or not os.path.exists(noise_dir):
        print(f"{common.bcolors.RED}data directory doesn't exist. Exiting...{common.bcolors.ENDC}")
        exit()
    ...
    op_clean_dir = os.path.join(processed_dir, "clean")
    op_noise_dir = os.path.join(processed_dir, "noise")
    op_noisy_dir = os.path.join(processed_dir, "noisy")
    if not os.path.exists(processed_dir):
        os.makedirs(processed_dir)
    if not os.path.exists(op_clean_dir):
        os.makedirs(op_clean_dir)
    if not os.path.exists(op_noise_dir):
